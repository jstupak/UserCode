...
void WprimeEventSelector::BeginJob( std::map<std::string, edm::ParameterSet const> par){
  BaseEventSelector::BeginJob(par);
    ...
    if ( par.find(_key)!=par.end() ){
        ...
        miPar["min_loose_muon"]           = par[_key].getParameter<int>          ("min_loose_muon");
        ...
        miPar["min_loose_electron"]       = par[_key].getParameter<int>          ("min_loose_electron");
	...
	mbPar["doQCD"]                    = par[_key].getParameter<bool>         ("doQCD");
    ...
    push_back("Min loose muon");
    push_back("Min loose electron");
    ...
    set("Min loose muon", miPar["min_loose_muon"]);
    set("Min loose electron", miPar["min_loose_electron"]);
...
bool WprimeEventSelector::operator()( edm::EventBase const & event, pat::strbitset & ret){
	...
	int nLooseLeptons = _n_loose_muons + _n_loose_electrons;
        ...
        if( nLooseLeptons >= cut("Min loose lepton", int()) || ignoreCut("Min loose lepton") ) passCut(ret, "Min loose lepton");
        else break;
        if( nLooseLeptons <= cut("Max loose lepton", int()) || ignoreCut("Max loose lepton") ) passCut(ret, "Max loose lepton");
        else break;
        ...
        if(_n_loose_muons >= cut("Min loose muon", int()) || ignoreCut("Min loose muon") ) passCut(ret, "Min loose muon");
        else break;
        if(_n_loose_electrons >= cut("Min loose electron", int()) || ignoreCut("Min loose electron") ) passCut(ret, "Min loose electron");
        else break;
	...
	if (mbPar["doQCD"]){
	  mvSelMuons=mvLooseMuons;
	  mvSelElectrons=mvLooseElectrons;
	}
